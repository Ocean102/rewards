...
searches.push(
  ...
  ...gamesWithMaps.flatMap(g => [
    ...mapTemplates.map(fn => fn(g)),
    ...gameTemplates.map(fn => fn(g))
  ]),
...
async function mainLoop() {
  ...
  let state = await new Promise(res => chrome.storage.local.get(['lastDate', 'searchPoints'], res))
  if (state.lastDate !== today) {
    state = { lastDate: today, searchPoints: 0 }
    chrome.storage.local.set({ lastDate: today, searchPoints: 0 })
  }
